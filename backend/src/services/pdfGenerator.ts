...
async function enforceSinglePageFit(page: Page): Promise<void> {
  ...
  const widthScale =
    layout.width > A4_PRINTABLE_WIDTH_PX * OVERFLOW_TOLERANCE
      ? widthFitScale
      : 1;
  const heightScale =
    layout.height > A4_PRINTABLE_HEIGHT_PX * OVERFLOW_TOLERANCE
      ? heightFitScale
      : 1;
  ...
  if (scale > 0.999 && scale < 1.001) return;

  await page.evaluate((calculatedScale: number) => {
    const doc = (globalThis as any).document;
    const body = doc?.body;
    if (!body) return;
...
export async function generateResumePDF(
  ...
  try {
    ...
    const filepath = path.join(GENERATED_DIR, profileSlug, dateStr, companySlug, roleSlug, 'Resume.pdf');
