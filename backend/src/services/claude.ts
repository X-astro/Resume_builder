...
async function createTextCompletion(prompt: string, provider: AIProvider = DEFAULT_PROVIDER, maxTokens = 4000, temperature = 0): Promise<string> {
  if (provider === 'openai') {
...
export async function analyzeJobDescription(jobDescription: string, provider: AIProvider = DEFAULT_PROVIDER): Promise<JobAnalysis> {
  const content = await createTextCompletion(
    `Analyze the following job description and extract key information for ATS optimization.
...
export async function tailorResume(
  ...
  const content = await createTextCompletion(
    `You are an expert resume writer focused on ATS optimization. Given a candidate's profile and job analysis, create tailored resume content that MAXIMIZES ATS score.
...
export async function generateCoverLetter(
  ...
  const content = await createTextCompletion(
    `You are an expert cover letter writer. Write a professional cover letter BODY for the following candidate applying to a job.
...
Return ONLY the cover letter body text, no JSON or other formatting.`,
    provider,
    1500,
    0.7
  );
...
export async function extractTemplateFromPDF(
  ...
  const content = await createTextCompletion(
    `Analyze this resume text and create a Handlebars HTML template that replicates its structure and layout.
...
export async function extractProfileFromResume(
  ...
  const content = await createTextCompletion(
    `Extract structured profile data from this resume text. Be thorough and extract ALL information.
