...
async function getAllTemplates() {
    await ensureDirectories();
    const files = await promises_1.default.readdir(TEMPLATES_DIR);
    const templates = [];
    for (const file of files) {
        if (file.endsWith('.json')) {
            const content = await promises_1.default.readFile(path_1.default.join(TEMPLATES_DIR, file), 'utf-8');
...
async function getTemplateById(id) {
    const templatePath = path_1.default.join(TEMPLATES_DIR, `${id}.json`);
    try {
...
<body>
  ...
  <div class="section-title">Education</div>
    ...
    const oneColumnTemplatePath = path_1.default.join(TEMPLATES_DIR, `${oneColumnTemplate.id}.json`);
    const existingOneColumn = await getTemplateById(oneColumnTemplate.id);
    const oneColumnToWrite = {
        ...oneColumnTemplate,
        disabled: existingOneColumn?.disabled ?? false,
        updatedAt: new Date().toISOString(),
    };
    await promises_1.default.writeFile(oneColumnTemplatePath, JSON.stringify(oneColumnToWrite, null, 2));
    const oneColumnModernTemplate = {
...
<body>
  ...
  <div class="section-title">Education</div>
    ...
    const oneColumnModernTemplatePath = path_1.default.join(TEMPLATES_DIR, `${oneColumnModernTemplate.id}.json`);
    const existingOneColumnModern = await getTemplateById(oneColumnModernTemplate.id);
    const oneColumnModernToWrite = {
        ...oneColumnModernTemplate,
        disabled: existingOneColumnModern?.disabled ?? false,
        updatedAt: new Date().toISOString(),
    };
    await promises_1.default.writeFile(oneColumnModernTemplatePath, JSON.stringify(oneColumnModernToWrite, null, 2));
